...
void MainWindow::closeEvent(QCloseEvent * event)
{
    ...
    if (!snapshotMan.isAllSaved())
    {
        auto clickedButton = QMessageBox::warning(
            this, "Closing RcMPPT-Configurator",
    ...
    if (settings.status() != QSettings::NoError)
    {
        ...
        if (settings.status() == QSettings::AccessError)
        {
            ...
            errorText = QString("RcMPPT-Configurator cannot save settings due to access error. \
This happens if you have run RcMPPT-Configurator as root (with sudo for ex.) previously. \
Try fixing the permissions of file: %1, or just delete it.").arg(file);
        }
        else
        {
            errorText = QString("RcMPPT-Configurator cannot save settings due to unknown error: %1").\
                arg(settings.status());
        }
